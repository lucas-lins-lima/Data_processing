...
def apply_anova_and_posthoc_to_means(tabela, data, question_col, group_column, column_prefix):
    ...
    # Tukey HSD com alpha = 0.10
    tukey = pairwise_tukeyhsd(endog=data_anova[question_col],
                               groups=data_anova[group_column],
                               alpha=0.10)

    comparisons = pd.DataFrame(data=tukey._results_table.data[1:], columns=tukey._results_table.data[0])
    groups = sorted(data_anova[group_column].dropna().unique())
    mean_vals = {g: data_anova[data_anova[group_column] == g][question_col].mean() for g in groups}
    ...
    for group in groups:
        # Procura o nome exato da coluna correspondente a esse grupo
...
with pd.ExcelWriter("Resultados_ANOVA+Tukey_HSD_Vitacon%Com_Teste.xlsx", engine="xlsxwriter") as writer:
