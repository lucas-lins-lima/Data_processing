...
def chi2_test_proportions(p1, n1, p2, n2):
    success1 = int(round(p1 * n1))
    success2 = int(round(p2 * n2))
    fail1 = n1 - success1
    fail2 = n2 - success2
...
def apply_chi2_group_labels(tabela, data, column_prefix, group_column, question_col, is_multi=False):
    from scipy.stats import chi2_contingency
    from statsmodels.stats.multitest import multipletests
    import string
    ...
    respostas = [i for i in tabela.index if 'Base' not in i and 'Média' not in i and 'Top' not in i and 'Bottom' not in i and 'Desvio' not in i]
    ...
    for resposta in respostas:
        ...
        for g in groups:
            grupo = data[data[group_column] == g]
            total_pessoas = grupo['_original_index'].nunique() if is_multi else grupo.shape[0]
            marcou = grupo[grupo['Resposta'] == resposta]['_original_index'].nunique() if is_multi else grupo[grupo['Resposta'] == resposta].shape[0]
            nao_marcou = total_pessoas - marcou

            cont_table.append([marcou, nao_marcou])
            grupo_sizes[g] = total_pessoas
            grupo_contagem[g] = marcou

        try:
            chi2, p_global, _, _ = chi2_contingency(cont_table)
        except:
            p_global = 1.0

        if p_global < 0.1:
            letras = {g: set(string.ascii_uppercase) for g in groups}
            comparacoes = []
            p_vals = []
            ...
            for i in range(len(groups)):
                for j in range(i+1, len(groups)):
                    ...
                    try:
                        _, pval, _, _ = chi2_contingency(tabela_2x2)
                        comparacoes.append((a, b))
                        p_vals.append(pval)
                    except:
                        continue

            _, pvals_corrigidos, _, _ = multipletests(p_vals, method='holm')

            for idx, (a, b) in enumerate(comparacoes):
                pval = pvals_corrigidos[idx]
                if pval < 0.05:
                    letras[a] &= letras[a] - letras[b]
                    letras[b] &= letras[b] - letras[a]
            ...
            for g in sorted(groups):
                disponiveis = letras[g] - usadas
                if disponiveis:
                    letra = min(disponiveis)
                else:
                    restantes = set(alfabeto) - usadas
                    letra = min(restantes) if restantes else 'Z'
                final_letras[g] = letra
                usadas.add(letra)

            for g in groups:
                col = f"{prefix}{g} (%)"
                try:
                    val = float(''.join([c for c in str(tabela.at[resposta, col]) if c.isdigit() or c == '.']))
                    tabela.at[resposta, col] = f"{val:.1f}% {final_letras[g]}"
                except:
                    continue

    return tabela

def apply_chi2_on_box_metrics(tabela, data, question_code, group_column, column_prefix):
    box_linhas = ['Topbox (5)', 'Top2Box (4+5)', 'Bottombox (1)', 'Bottom2Box (1+2)']
    grupos = sorted(data[group_column].dropna().unique())
    prefix = f"{column_prefix} " if column_prefix else ""

    for linha in box_linhas:
        if linha not in tabela.index:
            continue
        ...
        for g in grupos:
            grupo_data = data[data[group_column] == g]
            total = grupo_data.shape[0]

            if linha == 'Topbox (5)':
                count = (grupo_data[question_code] == 5).sum()
            elif linha == 'Top2Box (4+5)':
                count = grupo_data[question_code].isin([4, 5]).sum()
            elif linha == 'Bottombox (1)':
                count = (grupo_data[question_code] == 1).sum()
            elif linha == 'Bottom2Box (1+2)':
                count = grupo_data[question_code].isin([1, 2]).sum()
            else:
                continue

            cont_table.append([count, total - count])
            grupo_sizes[g] = total
            grupo_counts[g] = count
        ...
        if p_global < 0.1:
            letras = {g: set(string.ascii_uppercase) for g in grupos}
            comparacoes = []
            p_vals = []
            ...
            for i in range(len(grupos)):
                for j in range(i+1, len(grupos)):
                    ...
                    tabela_2x2 = [cont_table[i], cont_table[j]]
                    try:
                        _, pval, _, _ = chi2_contingency(tabela_2x2)
                        comparacoes.append((a, b))
                        p_vals.append(pval)
            ...
            # Aplica letras na tabela
            for g in grupos:
                col = f"{prefix}{g} (%)"
                try:
                    val = float(''.join([c for c in str(tabela.at[linha, col]) if c.isdigit() or c == '.']))
                    tabela.at[linha, col] = f"{val:.1f}% {final_letras[g]}"
                except:
                    continue
...
def analyze_questions(df, questions, filters):
    ...
    for q in questions:
        ...
        for f in question_filters:
            ...
            tabela = apply_chi2_group_labels(
                tabela,
                data,
                column_prefix=prefix,
                group_column=prefix,
                question_col=question_code,
                is_multi=isinstance(question_code, list)
            )
            # Teste de Qui-quadrado nas métricas Top/BottomBox (se aplicável)
            if not isinstance(question_code, list):  # só para perguntas escalares
               tabela = apply_chi2_on_box_metrics(
                  tabela,
                  data,
...
def export_tables_to_excel(tabelas, writer):
    ...
    for tabela, title, question_code, question_text, sheet in tabelas:
        ...
        worksheet.write(r_idx, 0, "A: significant difference at 95%", note_format)
        worksheet.write(r_idx + 1, 0, "a: significant difference at 90%", note_format)
        worksheet.write(r_idx + 2, 0, "Chi-square test with post-hoc grouping (letters)", note_format)
...
with pd.ExcelWriter("Resultados_Qui_Quadrado_Vitacon%Com_Teste.xlsx", engine="xlsxwriter") as writer:
